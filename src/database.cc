...
// add by chenbh
void set_key(const std::string& file, const std::string& psw){
    sqlite3* db = nullptr;
...
NAN_METHOD(Database::New) {
    ...
    Nan::ForceSet(info.This(), Nan::New("password").ToLocalChecked(), info[1].As<String>(), ReadOnly);
    Nan::ForceSet(info.This(), Nan::New("mode").ToLocalChecked(), Nan::New(mode), ReadOnly);

    // Start opening the database.
    // add by chenbh, encrypt the sqlite3
    set_key(*filename, *password);
    // end
...
void Database::Work_Open(uv_work_t* req) {
    ...
    baton->status = sqlite3_key(db->_handle, db->password.c_str(), db->password.size());
    // end
