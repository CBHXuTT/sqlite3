...
NAN_METHOD(Backup::New) {
    ...
    Local<String> password = Local<String>::Cast(info[2]);
    Local<String> sourceName = Local<String>::Cast(info[3]);
    Local<String> destName = Local<String>::Cast(info[4]);
    Local<Boolean> filenameIsDest = Local<Boolean>::Cast(info[5]);

    Nan::ForceSet(info.This(), Nan::New("filename").ToLocalChecked(), filename, ReadOnly);
    Nan::ForceSet(info.This(), Nan::New("password").ToLocalChecked(), password, ReadOnly);
    ...
    InitializeBaton* baton = new InitializeBaton(db, Local<Function>::Cast(info[6]), backup);
    baton->filename = std::string(*Nan::Utf8String(filename));
    baton->password = std::string(*Nan::Utf8String(password));
...
// add by chenbh
void set_backup_key(const std::string& file, const std::string& psw){
    sqlite3* db = nullptr;
    int mode = SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX;
    int result = sqlite3_open_v2(file.c_str(), &db, mode, nullptr);
    if(result != SQLITE_OK) return;
    result = sqlite3_rekey(db, psw.c_str(), psw.size());
    sqlite3_close(db);
}
...
void Backup::Work_Initialize(uv_work_t* req) {
    ...
    // add by chenbh
    set_backup_key(baton->filename, baton->password); 
    ...
    // add by chenbh, decrypt the sqlite3
    baton->status = sqlite3_key(backup->_otherDb, baton->password.c_str(), baton->password.size());
    // end
